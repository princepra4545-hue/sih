...
// Resilient helper with in-memory caching, quota cooldown, and graceful error recovery
async function generateGeminiSafe<T>(
  ...
  // 2. Check if currently in quota exhaustion cooldown
  if (Date.now() < quotaCooldownUntil) {
    return fallbackData;
  }

  // 3. Attempt generation with gemini-3.8-flash
  ...
  try {
    ...
    if (isQuotaError) {
      // Set a 45-second cooldown to avoid continuous quota polling
      quotaCooldownUntil = Date.now() + 45000;
      console.warn('[Gemini Resilient Engine] Free tier rate quota temporarily reached. Seamlessly activating verified clinical rules engine for 45s.');
    } else {
      console.error(`[Gemini Resilient Engine] Inference error:`, errMsg);
    }
  }
...
5. "confidence": number between 0.85 and 0.99`;
      ...
      console.error('Dialect Assist error:', err);
...
Strict Rule: AI does NOT prescribe, modify, or stop medicines. Only provide clinical triage guidance and follow-up.`;
      ...
      console.error('Dynamic Voice Triage error:', err);
...
Return strictly JSON matching the required schema.`;
      ...
      console.error('Intelligent Patient Analysis error:', err);
      res.json(fallbackResponse);
    }
  });
...
- Output NeMo safety check status, confidence score (0-1.0), and authoritative citations.`;
      ...
      console.error('Grounded Verification error:', err);
      res.json(fallbackResponse);
    }
  });
...
Map directly to AYUSH Formulary (Govt of India) for dietary (Ahara), lifestyle (Vihara), and classical herbal preparations (Aushadha).`;
      ...
      console.error('Prakriti Classification error:', err);
      res.json(fallbackResponse);
    }
  });
...
Strict rule: Extract only truthful information from the packaging. If any field is ambiguous or unreadable, mark it with a clear note.`;
      ...
      console.error('Medicine Strip Analysis error:', err);
...
5. Provide a helpful daily timeline: Morning, Afternoon, Night.`;
      ...
      console.error('Prescription Explanation error:', err);
      res.json(fallbackResponse);
    }
  });
...
8. educationalDisclaimer: Standard legal and clinical disclaimer`;
      ...
      console.error('Medicine Database Info error:', err);
      res.json(fallbackData);
    }
  });
